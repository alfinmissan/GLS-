...
import {ItemNoHistory, ItemNoReqHistory} from './History';
import DeleteButton from '../components/DeleteButton';

export const ItemBasedRequirement = () => {
 ...
 const ItemRequirements = () => axios({
  method: 'get',
  url: Url+'itemrequirement',
  params:{
   item_no:item_no
  },
}).then(res=>{
...
const DropDowns = () => axios.get(Url+'req/dropdown',{
 ...
 function download(val) {
  axios.defaults.headers.common['Authorization'] = Token
  axios.get(Url+`import/exportItemRequirements?item=${item_no}`,{
    'headers':{
      'Authorization': Token
    }
    }).then((res)=>{
...
const historyClose = () =>setShowhistory(false)
  return (
    <div className='blendbased_requirement hd'>
      <DeleteButton module="item_no_requirements" param={item_no}/>
        <MainNavbar prop='ITEM REQUIREMENTS'/>
          ...
          <div className="container">
            <div className="w3-show-inline-block" style={{float:"left"}}>
               ...
               <div className="w3-show-inline-block" style={{float:"right"}}>
                 <div className="w3-bar">
                 <Link to='/item/requirement/import'><button className="w3-btn hd-btns">IMPORT</button></Link>
        ...
        <Modal show={showhistory} onHide={historyClose} dialogClassName='custom-dialog-history' scrollable>
            ...
            <Modal.Body>
                <ItemNoReqHistory itemno={item_no}/>
            </Modal.Body>
       </Modal>
        <AddItemBaseRequirement show={show} setShow={setShow} item_no={item_no}/>
       {modifyshow && <ModifyItemRequirement modifyshow={modifyshow} setModifyShow={setModifyShow} countrylist={countryList} item_no={item_no} item={item} val={value} countrydict={country}/>}
...
const AddItemBaseRequirement = (props) => {
    const {show,setShow,item_no} = props
  ...
  return (
    <Modal show={show} onHide={handleClose} scrollable>
      <NotificationContainer/>
       <SuccessMessage ssShow={ssShow} setSsShow={setSsShow} message={message}/>
    <Modal.Header closeButton>
    <Modal.Title>ADD ITEM REQUIREMENT</Modal.Title>
    </Modal.Header>
    <Modal.Body>
...
const ModifyItemRequirement = (props) =>{
    const {modifyshow,setModifyShow,item,val,countrydict,item_no} = props
...
return(<Modal show={modifyshow} onHide={handleClose}>
       <Modal.Header closeButton>
        <NotificationContainer/>
        <SuccessMessage ssShow={ssShow} setSsShow={setSsShow} message={message}/>
       <Modal.Title>MODIFY ITEM REQUIREMENT</Modal.Title>
       </Modal.Header>
       <Modal.Body>
       <Form onSubmit={handleSubmit}>
       <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
       ...
       <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
            <Form.Label>Type :</Form.Label>
            <select style={{width:"15vw",height:"6vh"}} id='test' value={type} onChange ={Type}>
            <option value="Plain-text">Plain Text</option>
            <option value="Translation-ID">Translation ID</option>
            <option value="Asset">Asset</option>
            {/* <option value="Asset">Asset</option> */}
...
export default ItemBasedRequirement
