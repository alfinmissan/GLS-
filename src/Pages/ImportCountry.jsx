...
class CountryExcelimport extends Component {
  constructor(props){
    super(props);
    this.state={
      isOpen: false,
  ...
  handleImport = (event) =>{
    ...
    const url = Url+'country/exprtimport';
    ...
    return  axios.post(url, formData,config).then(res=>{
      ...
      if(res.status==200){
        ...
        NotificationManager.error("Import rejected ,language Code " +res.data +" not present in database")
      }
    })
  ...
  render() {
    return (
      <div style={{"marginTop":"13vh"}}>
        <MainNavbar prop='Import Country'/>
        {/* <div>
        ...
        <div>
          <p style={{color:"red"}}>Import Rejected Country already present in Database</p>
          <Card body outline color="secondary" className="restrict-card">
...
export default CountryExcelimport;